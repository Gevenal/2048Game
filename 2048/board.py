...
def hex_to_rgb(hex_color):
	...
	class Board:
		def _init_(self,rows,cols,game_state,padding,tile_size,
			backgound_color,empty_tile_color,background_tile_colors,tile_colors,font)
		    self._rows=rows
		    self._cols=cols
        ...
        def draw_board(self)
            ...
            for col_num in range(self._cols):
            	...
            	row.bilt(tile,(self._padding+col_num*(self._padding+self._tile_size),
            		self._padding))
            for row_num in range(self._rows):
            	self._surface.bilt(rows,(0,self._padding+self._tile_size)*row_num)
